In questa sezione viene assegnato un codice ad ogni requisito al fine di facilitarne l'identificazione. L'assegnazione avverrà secondo le condizioni stabilite dal gruppo nel documento \textit{NormeDiProgetto\_v4.0.0}, riportate in seguito. 
...
\begin{tabularx}{\textwidth}{| c | p{10cm} | X |}
		...
		RDF16 & Il moderatore deve visualizzare nome, cognome e username dell'utente in lista. & UC-M5.1 \newline Interno\\
		...
		RPF26 & Lo sviluppatore può visualizzare le informazioni relative ad un dataset. & UC-S10 \newline Interno\\
		RDF40 & Lo sviluppatore può visualizzare la lista dei modelli disponibili. & UC-S11 \newline Interno\\
		RDF41 & Lo sviluppatore deve visualizzare il nome e la lingua del modello in lista. & UC-S1.1 \newline Interno\\
		RPF27 & Lo sviluppatore deve poter scaricare un modello. & UC-S12 \newline Capitolato\\
		RPF28 & Lo sviluppatore può visualizzare informazioni riguardanti i modelli disponibili nella piattaforma. & UC-S13 \newline Interno\\
		RPF29 & Lo sviluppatore deve poter creare un modello tramite la piattaforma. & UC-S14 \newline Capitolato\\ 
		RDF42 & Lo sviluppatore visualizzerà un messaggio di errore se inserirà un dataset in un formato errato. & UC-S15 \newline Interno\\
		RPF30 & Lo sviluppatore può cambiare modello utilizzato dal software di apprendimento automatico. & UC-S16 \newline Interno\\
		RDF43 & Lo sviluppatore visualizzerà un messaggio di errore se indicherà un'intervallo di voti errato. & UC-S17 \newline Interno\\
		\hline
...
\begin{longtable}{| c | p{10cm} | c |}
		...
		ROQ3 & Tutte le norme stabilite nel documento \textit{NormeDiProgetto\_v4.0.0} devono essere rispettate & Interno\\
		RDQ4 & Il numero di linee di codice di ogni procedura non deve essere maggiore di 20. & Capitolato\\
		RDQ5 & Il valore della complessità ciclomatica$^*$ (stabilito nelle \textit{NormeDiProgetto\_v4.0.0}) deve essere minore di 10. & Capitolato\\
		\hline

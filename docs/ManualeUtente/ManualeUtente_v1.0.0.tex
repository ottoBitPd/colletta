...
\begin{document}
	...
	La piattaforma è provissoriamente raggiungibile solo tramite server locale.
	Per creare un server locale, eseguire i seguenti passaggi:
	 \begin{enumerate}
	 	\item \textbf{Installare le librerie necessarie:}\\ Da terminale, posizionarsi nella cartella principale, aprire la shell e utilizzare il seguente comando:\begin{lstlisting}[numbers=none]
	 	npm install
	 	\end{lstlisting}
 ...
 \newpage
	...
	\subsection{Utente registrato}
		...
		\begin{enumerate}
			\item Effettua l'accesso alla piattaforma;
			\item Clicca sul tuo username in alto a sinistra;
			\item Modifica i campi di tuo interesse;
			\item Clicca su "Conferma".
		\end{enumerate}

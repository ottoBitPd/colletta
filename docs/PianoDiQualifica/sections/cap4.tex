...
\begin{longtable}{|p{2cm}|p{2cm}|p{6.25cm}|p{4cm}|}		
	\rowcolor{LightBlue}
		...
		TS-RO1
		& ROF1 
		& Verifica che l'utente si registra alla piattaforma creando un account personale. 
		& non implementato\\ \hline
		\rowcolor{white}
		TS-RO2
		& ROF2 
		& Verifica che l'utente esegue l'accesso alla piattaforma utilizzando le sue credenziali. 
		& non implementato\\ \hline
		\rowcolor{LightGray}
		& RDF1 
		& Verifica che l'utente modifica i dati del proprio profilo personale. 
		& non implementato\\ \hline
		\rowcolor{white}
		& RDF2 
		& Verifica che l'amministratore verifica le credenziali di un utente che richiede la registrazione come insegnante 
		& non implementato\\ \hline
		\rowcolor{LightGray}
		& RDF3 
		& Verifica che l'utente viene avvisato in caso di errore nell'inserimento dei dati richiesti. 
		& non implementato\\ \hline
		\rowcolor{LightGray}
		& ROF3 
		& Verifica che l'insegnante e l'allievo devono poter ricercare degli esercizi sulla piattaforma. 
		& non implementato\\ \hline
		\rowcolor{LightGray}
		& RPF1 
		& Verifica che durante la ricerca, l'utente ha la possibilità di impostare dei filtri per raffinarla. 			
		& non implementato\\ \hline
		\rowcolor{LightGray}
		& RDF4 
		& Verifica che dopo una ricerca, l'utente viene avvisato con un messaggio se il sistema non ha trovato nessun risultato che corrisponde ai criteri selezionati. 
		& non implementato\\ \hline
		\rowcolor{LightGray}
		& RPF2 
		& Verifica che l'amministratore può eliminare un utente iscritto alla piattaforma. 
		& non implementato\\ \hline
		\rowcolor{LightGray}
		& RPF3 
		& Verifica che l'insegnante può modificare una soluzione di un esercizio da lui fornita. 
		& non implementato\\ \hline
		\rowcolor{LightGray}
		& RDF5 
